...
object ClearMethodMatcher : MethodMatcher {
    private const val METHOD_CLEAR = "clear"
    override fun match(symbol: Symbol.MethodSymbol): Boolean {
        if (symbol.getAnnotation(KVClear::class.java) == null && symbol.name.toString() != METHOD_CLEAR
        ) {
            return false
        }
    ...
    override fun buildMethod(symbol: Symbol.MethodSymbol): MethodSpec.Builder {
        return MethodSpec.methodBuilder(symbol.name.toString()).addModifiers(Modifier.PUBLIC)
            ...
            .addStatement("\$L.\$L()", KVImplTaskRunner.FIELD_PROVIDER, METHOD_CLEAR)
    }
}